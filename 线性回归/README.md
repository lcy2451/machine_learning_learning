...
`lr_torch_3f.py` 当前选取行程里程 `TRIP_MILES`、行程时长 `TRIP_SECONDS` 和公司 `COMPANY` 三列作为输入，以车费 `FARE` 为目标。公司名称先统一大写、去掉逗号并清理首尾空格，再将名称中每个字符的 Unicode 编码（`ord()`）相加，最终转换为浮点数。这种方式将公司信息保留为一列，不使用独热编码。
...
当前脚本已构造三列输入和对应的线性模型，但 `train()` 调用仍被注释，尚未启用三特征训练流程。
